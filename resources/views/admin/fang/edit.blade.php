...
@section('cnt')
    <nav class="breadcrumb">
        ...
        <span class="c-gray en">&gt;</span> 添加修改
        ...
        <form action="{{ route('admin.fang.update',$fang) }}" method="post" class="form form-horizontal" id="fang-add">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-4 col-sm-4">
                    ...
                    <select name="fang_region" id="fang_region" style="width: 100px;">
                        @foreach($currentRegionData as $item)
                            <option @if($item->id == $fang->fang_region) selected @endif value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="formControls col-xs-4 col-sm-5">
                    <input type="text" class="input-text" name="fang_addr" value="{{$fang->fang_addr}}" placeholder="小区详情地址和房源说明">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>租金：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" style="width: 200px;" name="fang_rent" value="{{$fang->fang_rent}}"> 元
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>楼层：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" style="width: 200px;" name="fang_floor" value="{{$fang->fang_floor}}">
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_rent_type" style="width: 100px;">
                        @foreach($fang_rent_type_data as $item)
                            <option @if($item->id == $fang->fang_rent_type) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_direction" style="width: 100px;">
                        @foreach($fang_direction_data as $item)
                            <option @if($item->id == $fang->direction) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_rent_class" style="width: 100px;">
                        @foreach($fang_rent_class_data as $item)
                            <option @if($item->id == $fang->fang_rent_class) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    @foreach($fang_config_data as $item)
                        <label>
                            <!--访问器-->
                            <input @if(in_array($item->id,$fang->fang_conf)) checked @endif type="checkbox" name="fang_config[]" value="{{ $item->id }}" />
                            {{ $item->name }} &nbsp;&nbsp;&nbsp;
                        </label>
                    @endforeach
                </div>
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <select name="fang_owner" style="width: 200px;">
                        @foreach($ownerData as $item)
                            <option @if($item->id == $fang->fang_owner) selected @endif value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-6 col-sm-7">
                    ...
                    <!-- 显示上传成功后的图片容器 -->
                    <div id="imglist">{!! $fang->images !!}</div>
                </div>
            </div>
            <div class="row cl">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <textarea name="fang_body" id="fang_body" cols="30" rows="10">{{ $fang->fang_body }}</textarea>
                </div>
            </div>
